  ...
  if (msg.type === 'pasteToTextbox') {
    ...
    figma.loadFontAsync({ family: 'Inter', style: 'Regular' }).then(() => {
      ...
      lines.forEach((line: string, index: number) => {
        ...
        const viewportScroll = figma.viewport.bounds;
        
        const scrollX = viewportScroll.x;
        const scrollY = viewportScroll.y;
        console.log(scrollX,scrollY,index);  

            // Set the position of the text node relative to the viewport scroll
            textNode.x = scrollX /2; // Example x coordinate offset from viewport scroll
            textNode.y = scrollY /2;
            textNode.characters = line;
            console.log(textNode.x,textNode.y);  
